...
int main() {
    ...
    int L;
    bool answer = false;
    // if (s.size()%2==0 && h.substringHash(0, s.size()/2-1)==h.substringHash(s.size()/2, s.size()-1)) {
    //     cout << "NO" << "\n";
    //     return 0;
    // }
    for (int i = s.size()/2 + 1; i<s.size(); i++) {
        ...
        if (h.substringHash(k, i-1)==h.substringHash(i, s.size()-1)) {
            int overlapLen = s.size() - i;
            if (s.compare(i, overlapLen, s, k, overlapLen) != 0)
                continue;

            if (s.compare(0, k, s, i - k, k) != 0)
                continue;
