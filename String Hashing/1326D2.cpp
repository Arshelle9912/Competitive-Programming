...
int main() {
    ...
    while (t--) {
        ...
        int left(0);
        while (left<n && h1.substringHash(0, left)==h2.substringHash(0, left)) {
            left++;
        }
        int leftToRight = left;
        int rightToLeft = left;
        for (int i = left; i<n-left; i++) {
            if (h1.substringHash(left, i)==h2.substringHash(n-i-1, n-left-1)) {
                leftToRight = i;
            }
        }
        for (int i = left; i<n-left; i++) {
            if (h2.substringHash(left, i)==h1.substringHash(n-i-1, n-left-1)) {
                rightToLeft = i;
            }
        }
        reverse(s.begin(), s.end());
        ...
        string P = s.substr(0, left);
        string S = s.substr(n - left, left);

        int L = leftToRight - left + 1;
        int R = rightToLeft - left + 1;

        if (L >= R) {
            string M = s.substr(left, L);
            cout << P << M << S << "\n";
        } else {
            string M = s.substr(n - left - R, R);
            cout << P << M << S << "\n";
        }
    }
}