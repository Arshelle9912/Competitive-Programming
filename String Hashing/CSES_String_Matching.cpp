...
int main() {
    ...
    struct Hashing h2 = Hashing(sub);
    auto hashesSub = h2.substringHash(0, sub.size() - 1);
    ll result(0);
    for (long long i = 0; i+sub.size()-1<s.size(); i++) {
        if (hashesSub==h1.substringHash(i, i+sub.size()-1)) {
            result++;
        }
    }
    cout << result << "\n";
