...
struct Hashing{
    ...
    pair<ll, ll> substringHash(int l, int r){
        ...
        for(int i = 0; i < 2; i++){
            ...
            i == 0 ? hash.first = mod_mul(mod_sub(val1, val2, hashPrimes[i]), inversePowersOfBase[i][l], hashPrimes[i])
            : hash.second = mod_mul(mod_sub(val1, val2, hashPrimes[i]), inversePowersOfBase[i][l], hashPrimes[i]);
        }
        return hash;
    }
};
...
int main() {
    ...
    Hashing h = Hashing(s);
    int n = s.length();
    unordered_set<pair<ll,ll>, PairHash> se;
    se.reserve((size_t)n * (n+1) / 2);
    int sum(0);
    ll result(0);
    for (int i = 0; i<n; i++) {
        sum += (binary[s[i]-'a']-'0');
        while (sum+k<i-left+1) {
            sum -= (binary[s[left]-'a']-'0');
            left++;
        }
        int temp = left;
        while (temp<=i) {
            pair<ll, ll> check = h.substringHash(temp, i);
