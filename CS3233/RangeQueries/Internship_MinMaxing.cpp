...
struct LazySegmentTree {
    ...
    int n;
    int s;
    LazySegmentTree(int a_len, vector<ll> &a) { // change if type updated
        arr = a;
        n = a_len;
        s = 1;
        while(s < 2 * n){
...
int main() {
    ...
    vector<ll> newArr(n-k+1);
    ll currSum = 0;
    for (int i = 0; i<n; i++) {
        currSum+=arr[i];
        if (i-k>=0) currSum-=arr[i-k];
        if (i-k+1>=0) {
            newArr[i-k+1] = currSum;
            // cout << currSum << "\n";
        }
    }
    LazySegmentTree<Node1, Update1> st(n-k+1, newArr);
    cout << st.make_query(0, n-k).val << "\n";
    for (int i = 0; i<q; i++) {
        ll day, value;
        cin >> day >> value;
        ll diff = value-arr[day-1];
        st.make_update(max(0LL, day-k), min(day-1, n-k), diff);
        arr[day-1] = value;
        cout << st.make_query(0, n-k+1).val << "\n";
    }
}