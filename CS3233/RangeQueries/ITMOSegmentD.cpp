...
struct SegmentTree {
    ...
    int find_first(int start, int end, int index, int left, ll x) {
        if (end<left || tree[index].val<x) return -1;
        if (start==end) return start;
        int mid = (start+end)/2;
        int res = find_first(start, mid, 2*index, left, x);
        if (res!=-1) {
            return res;
        }
        return find_first(mid+1, end, 2*index+1, left, x);
    }
};
...
int main() {
    ...
    for (int i = 0; i<q; i++) {
        cin >> type >> num1 >> num2;
        if (type==1) {
            st.make_update(num1, num2);
        } else {
            int low = num2;
            int x = num1;
            cout << st.find_first(0, n-1, 1, low, x) << "\n";
        }
    }
}