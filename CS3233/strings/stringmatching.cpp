...
int main() {
    ...
    while (getline(cin, pattern)) {
        getline(cin, text);
        Hashing h1 = Hashing(pattern);
        Hashing h2 = Hashing(text);
        int len = pattern.size();
        ll patternHash = h1.substringHash(0, len-1);
        int tot = text.size();
        for (int i = 0; i<tot-len+1; i++) {
            if (patternHash==h2.substringHash(i, i+len-1)) {
                cout << i << " ";
            }
        }
        cout << "\n";
