...
bool checker(vector<pair<double, double>>& blue, vector<pair<double, double>>& red, double d, int n) {
    int b = blue.size();
    int r = red.size();
    BipGraph g(b, r);
    for (int i = 0; i<b; i++) {
        for (int j = 0; j<r; j++) {
            if (dist(blue[i], red[j])<=d) g.addEdge(i+1, j+1);
        }
    }
    return (b+r-g.hopcroftKarp())>=n;
}
// Driver Program
int main()
{
